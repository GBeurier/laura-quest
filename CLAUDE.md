...
La musique est jouée par `js/music.js` (`window.LQ_MUSIC`), un lecteur **MIDI maison volontairement lo-fi** : parseur SMF + synthèse Web Audio (ondes carrées/triangle, bruit pour les percussions canal 9) — zéro lib, zéro réseau, marche en `file://`. Les sources sont des **`.mid` dans `assets/music/`** (quelques Ko pièce), embarqués en base64 par `gen_assets_data.py` (préfixe `_` = ignoré, comme les sprites). **Une piste par niveau** (nom = clé du niveau : `niveau1.mid`…`jury.mid`, override possible via `LEVELS[x].music`) **+ une par écran** via la table `CONFIG.music`. **Tous les écrans menu/narratifs (`title`, `slots`, `overworld`/map, `chapter`) partagent la piste `title`** : comme rejouer la piste courante est un no-op, la musique du titre **ne se coupe pas** en passant de l'un à l'autre ; seuls `win`, `lose` et les niveaux ont leur propre piste (les anciens `map.mid`/`chapter.mid` ont été retirés). Piste absente = silence (rien ne casse). Chaque scène appelle `playMusic(...)` à son ouverture ; rejouer la piste courante est un no-op (pas de coupure quand deux scènes partagent une piste, ex. title↔slots↔map↔chapter — entrer dans un niveau bascule sur la piste du niveau, donc revenir au chapitre/à la carte redémarre `title`). `M` coupe/rétablit (persisté), `CONFIG.audio.musicVolume` règle le volume. Placeholders : `python3 tools/gen_placeholder_midis.py` (puis `gen_assets_data.py`) — remplace-les par tes vrais `.mid` (mêmes noms). NB : l'AudioContext ne démarre qu'au **premier geste utilisateur** (politique autoplay des navigateurs) — l'écran titre s'en charge.
...
### Scene flow
`title → slots → overworld → game → chapter → overworld … → jury → win`. **Mort (v4)** : séquence in-scene (cadavre de Laura au sol — sprite `hero_dead` si présent, sinon repli basculé —, flash rouge + message `story.dead`) ; le cadavre est posé sur le sol **sous les pieds** (`surfaceUnderFeet`, jamais sur un deck de panneaux au-dessus) + léger enfoncement. Puis (≈1,2 s) un **popup** (`openDeathBox`, armé par scène via `openDeathBox` module-level) propose **deux choix** : *Recommencer le niveau* (`go('game', …)`, arsenal RESET) ou *Reprendre au boss* (arme `LEVEL.checkpoint` juste devant l'arène + `respawnAtArena` **in-scene** → arsenal **conservé**, pas de re-traversée). Sélection ◀▶, validation ESPACE/ENTREE/saut/OK ou tap ; gèle via `setPaused`. Défaut présélectionné = *boss* si Laura est morte dans la zone du boss (gates d'arène posées / x dans l'arène), sinon *niveau*. L'option boss n'apparaît que si un boss vivant existe (`LEVEL.bossX != null && bossesAlive > 0`). La scène `lose` reste définie mais n'est plus atteinte ; l'auto-respawn `CONFIG.bossCheckpoint` (false) est remplacé par ce choix. Menu scenes confirm via the `onConfirm` helper (space + enter + jump keys); `overworld` navigates levels with left/right.

### Levels = ASCII grids (`js/level.js`)
Each char is one `CONFIG.tileSize` tile; the legend is documented at the top of the file. **Perf** : les tuiles de sol `'='` n'ont PAS de collision individuelle — `addSolid` ne pose que le sprite (culle hors-ecran) et `addGroundColliders` (buildLevel) pose des **colliders invisibles fusionnes par segment horizontal** (meme geometrie AABB, ~25 objets physiques au lieu de ~600). `'='` n'est jamais retire de la grille en cours de partie (seuls les panneaux `x` explosent), donc les colliders n'ont pas a la suivre. Camera scrolls **X only by default** — 11 rows are visible (the **bottom** 11: taller maps put secret decks above the screen, reached blind — see niveau5). **Zone-ciel (`'|'`, caméra verticale)** : poser des `'|'` dans la map définit une **plage X** (de la 1re à la dernière colonne marquée) où la caméra **suit Laura vers le haut** (révèle un niveau caché en hauteur, garde Laura ~sur la ligne d'horizon), et **se reverrouille en bas** passé la dernière colonne. Implémenté dans `setCam` (boucle d'update, game.js) via `LEVEL.baseCamY` (vue épinglée bas, calculée dans `buildLevel`) et `LEVEL.camY` (position courante lissée) ; `LEVEL.skyZone = {fromX, toX}` posé par le parseur. Le **parallax suit en vertical** : `addBandLayer`/`addScatterLayer` décalent les couches de `(baseCamY - camY) × vpar` (`vpar` = parallax vertical, défaut = horizontal `parallax`, sauf le panorama **intérieur** `band y:'ground'` → vpar 1 = défile hors-champ en montant pour découvrir le ciel). Les niveaux **intérieurs** avec zone-ciel reçoivent un **plafond-ciel** (`addSkyCeiling` : ciel bleu + `bg_mountains` + `bg_cloud` ancrés au MONDE, derrière le panorama `z<-22`), réglable via `theme.ceiling { row|worldY, sky, mountains, clouds, tint, mDrop, ... }`. Le `'|'` ne pose rien d'autre sur sa case. The hidden publication (`P`, one per level, gives 100%) is **gated by the level's gear**: niveau1 = double-jump panel stair, niveau2/4 = vélo gap-cross (gap ≥8 at h7, no mid stepping stones), niveau3/5 = roller float (P at h≥8 over a CLEAN column). The `'B'` tile spawns the boss named by the level's `boss:` field; touching `'*'` after killing the boss writes the chapter.
