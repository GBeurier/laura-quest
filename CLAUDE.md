...
### Scene flow
`title → slots → overworld → game → chapter → overworld … → jury → win`. **Mort (v3)** : séquence in-scene (cadavre de Laura au sol — sprite `hero_dead` si présent, sinon repli basculé —, flash rouge + messages `story.dead`/`deadSub`) puis la scène `game` **redémarre le niveau** toute seule (`deathSequence`, game.js) ; la scène `lose` reste définie mais n'est plus atteinte depuis le jeu. Menu scenes confirm via the `onConfirm` helper (space + enter + jump keys); `overworld` navigates levels with left/right.
